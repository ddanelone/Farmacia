...
public class PurchasesDao {
    ...
    try {
            ...
            JOptionPane.showMessageDialog(null, "Error al registrar los detalles la compra");
            return false;
        }
    }
    ...
    public List listAllPurchasesQuery() {
        ...
        String query = "SELECT pu.*, su.name AS supllier_name FROM purchases pu, suplliers su "
    ...
    //Listar compras para imprimer factura
    public List listAllPurchaseDetailQuery(int id) {
        ...
        String query = "SELECT pu.created, pude.purchase_price, pude.purchase_amount, pude.purchase_subtotal, "
                 + "su.name AS supplier_name, pro.name AS product_name, em.full_name FROM purchases pu "
                + "INNER JOIN purchase_details pude ON pu.id = pude.purchase_id INNER JOIN products pro ON pude.product_id = pro.id "
                + "INNER JOIN suppliers su ON pu.supplier_id = su.id INNER JOIN employees em ON pu.empoloyee_id = em.id WHERE pu.id = ?";
        ...
        try {
            ...
            while(rs.next()) {
                Purchases purchase = new Purchases();
                purchase.setProduct_name(rs.getString("pruduct_name"));
